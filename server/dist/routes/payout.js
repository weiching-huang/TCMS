...
router.get("/", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
        ...
        if (month)
            filter.month = month;
        const payouts = await Payout.find(filter).populate("teacher", "name email");
        res.json(payouts);
    }
    catch (error) {
...
// 查詢單筆自己的 payout 詳細內容
router.get("/me/:id", passport.authenticate("jwt", { session: false }), async (req, res) => {
    try {
        const user = req.user;
        if (user.role !== "teacher") {
            return res
        ...
        if (!payout)
            return res.status(404).json({ message: "找不到該 payout" });
        // 確保只能查自己的
...
router.get("/:id", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
        const payout = await Payout.findById(req.params.id).populate("teacher", "name email");
        if (!payout)
            return res.status(404).json({ message: "找不到該 payout" });
        res.json(payout);
    }
...
// 觸發結算程序
router.post("/settle", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
        const now = new Date();
        ...
        const settlementMonth = `${startDate.getFullYear()}-${(startDate.getMonth() + 1)
            .toString()
            .padStart(2, "0")}`;
        // 取得該月份所有上課紀錄
        const attendances = await Attendance.find({
            date: { $gte: startDate, $lte: endDate },
        }).populate("course", "teacher fee totalSessions teacherShareRatio adminShareRatio");
        // 用 Map 按老師統計
        const teacherMap = new Map();
        for (const att of attendances) {
            const course = att.course;
            if (!course)
                continue;
            ...
            const attendedCount = att.records.filter((r) => r.status === "present").length;
...
router.patch("/:id", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
