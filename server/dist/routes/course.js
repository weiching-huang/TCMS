...
router.post("/", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
        const { title, description, teacher, maxStudents, totalSessions, fee, materials, teacherShareRatio, adminShareRatio, } = req.body;
        // 檢查比例是否正確
...
//管理者刪除課程
router.delete("/:courseId", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
...
//管理者更新課程
router.put("/:courseId", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
        // 檢查更新內容中的分帳比例
        const { teacherShareRatio, adminShareRatio, ...rest } = req.body;
        if (teacherShareRatio !== undefined &&
        ...
        const updatedCourse = await Course.findByIdAndUpdate(req.params.courseId, req.body, { new: true, runValidators: true }).populate("teacher", "_id name email");
        if (!updatedCourse) {
            return res.status(404).json({ success: false, message: "課程不存在" });
        }
...
router.get("/", passport.authenticate("jwt", { session: false }), async (req, res) => {
    try {
        ...
        const courses = await Course.find(query)
            ...
            .select("title description fee teacher maxStudents totalSessions status")
            .skip(skip)
            .limit(Number(limit));
...
//管理者查看該課程的所有學生
router.get("/:courseId/students", passport.authenticate("jwt", { session: false }), async (req, res, next) => {
    const user = req.user;
    if (user.role === "admin") {
        return next(); // 管理員直接通過
    }
...
// 查詢單一課程的所有付款紀錄 (管理員)
router.get("/:courseId/payments", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
...
//查詢未付款學生
router.get("/:courseId/unpaid", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
        const { courseId } = req.params;
        //檢查課程是否存在
        const course = await Course.findById(courseId);
...
// 學生繳費(管理員才能點選)
router.put("/:courseId/pay", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
        const { courseId } = req.params;
...
// 學生退款 (管理員才能點選)
router.put("/:courseId/refund", passport.authenticate("jwt", { session: false }), isAdmin, async (req, res) => {
    try {
        const { courseId } = req.params;
        const { studentId } = req.body;
        // 找 enrollment
